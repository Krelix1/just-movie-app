...
let SignIn = (props) => {
    ...
    let fireLogin = (value) => {
        props.Login({email: value.email, password: value.password}, props);
    };
...
const LoginForm = (props) => {
    function loginWithGoogle() {
        props.LoginWithNetworks({provider: 'google', type: 'popup'},props);
    }
        ...
        : <div className={css.formWrapper}>
            <form onSubmit={props.handleSubmit} className={css.form}>
                ...
                <Field component={"input"} type={"email"} name={"email"}
                       placeholder={props.language === "en-US" ? "Mail" : "Почта"}/>
...
let mapDispatchToProps = (dispatch) => ({
    ...
    LoginWithNetworks: (credentials, ownProps) => {
        dispatch(LoginWithNetworks(credentials, ownProps));
    }
});

const ReduxLoginForm = compose(reduxForm({form: "SignIn"}), withFirebase, withFirestore, connect(mapStateToProps, mapDispatchToProps))(LoginForm);
